    ...
    <div class="content" style="margin-top:60px;padding:0 3px;" >
        @for( $i=0;$i<count($list);$i++ )
            <div data="{{$list[$i]->color}}&{{$list[$i]->chicun}}&{{$list[$i]->num}}#**#{{$list[$i]->addr}}#**#{{$list[$i]->liuyan}}#**#{{$list[$i]->phone}}#**#{{$list[$i]->otime}}#**#{{$list[$i]->orderid}}" style="height:120px;width:100%;position: relative;margin-bottom:20px;position: relative;" >
                ...
                @if($list[$i]->orderid == '0')
                    <a href='javascript:void(0)' order_id='{{$list[$i]->id}}' p='{{$list[$i]->price}}' num='{{$list[$i]->num}}' type='shop' onclick='payment(this,null,null)'  style='position: absolute;right:0px;bottom:5px;background: #E83888;color:#fff;display:inline-block;text-decoration: none;cursor: pointer;text-align: center;font-size:14px;padding:4px 8px;'>立即支付</a></div>
                @else
