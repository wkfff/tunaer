...
function gettubuorder(userid) {
    ...
    <a href="/tubu/tubudetail/${res[i].tid}"><p style="color:#1E90FF;font-size:16px;overflow: hidden;text-overflow:ellipsis;white-space: nowrap;display:inline-block;max-width:70%;position: absolute;left:0px;top:5px;">${res[i].title}</p></a>
                ...
                if( res[i].orderid == 0 ) {
                    var tmp = "<a href='javascript:void(0)' order_id='"+res[i].id+"' p='"+res[i].price+"' num='"+res[i].num+"' type='tubu' onclick='payment(this,null,null)'  style='position: absolute;right:0px;top:5px;background: #E83888;color:#fff;display:inline-block;text-decoration: none;cursor: pointer;text-align: center;font-size:14px;padding:4px 8px;'>立即支付</a></div>";
                }else{
